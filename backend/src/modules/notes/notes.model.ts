...
// ownership guarantee (PRD §5.11 + §4). A caller can never pass another
// user's id in; it always comes from the verified JWT, never a request param.
...
// notes.user_id is polymorphic (PRD §5.11): assert_user_exists (the
// trigger), not a foreign key, checks it. No pre-check here.
...
// from the response, which is the point of the 404-not-403 rule (PRD §2.4).
...
export async function deleteOwnNote(noteId: number, userId: number): Promise<boolean> {
  const { rowCount } = await pool.query(`DELETE FROM app.notes WHERE note_id = $1 AND user_id = $2`, [
    noteId,
    userId,
  ]);
  return (rowCount ?? 0) > 0;
