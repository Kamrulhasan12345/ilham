...
| `sanad_strengths` | A **VIEW**, not a table. It exposes the per-sanad strength that `corpus.chain_strength` aggregates to its maximum. The API reads it for the grouped chains in `GET /hadiths/:id`. `db/07_sanad_strength.sql` creates it |
