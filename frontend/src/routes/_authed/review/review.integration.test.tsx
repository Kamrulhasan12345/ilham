...
describe('Review runner integration', () => {
  it('submits every verdict in one transaction and lands on the session record', async () => {
    vi.mocked(apiFetch).mockImplementation(async (path: string, _schema: unknown, options?: { body?: unknown }) => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: 'Passed' })[0]);
    fireEvent.click(screen.getAllByRole('button', { name: 'Partial' })[1]);
    ...
    await waitFor(() =>
      expect(router.state.location.pathname).toBe('/review/42'),
    );
  });
  ...
  it('refuses to submit before every hadith has a verdict', async () => {
    ...
    expect(await screen.findByText(/Hadith 0 of 2/)).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Decide every hadith first/ }),
    ).toBeDisabled();
