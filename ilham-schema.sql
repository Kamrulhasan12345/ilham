...
-- Uses table inheritance for role-based specialization (IS-A relationship)
...
-- Subtypes: role-specific tables with inheritance
-- Students: learners who enroll in circles and track progress
CREATE TABLE app.students (
    student_level text CHECK (student_level IN ('beginner','intermediate','advanced'))
) INHERITS (app.users);

-- Teachers: instructors who run circles and review student work
CREATE TABLE app.teachers (
    institution    text,
    specialization text  -- e.g., "Bukhari specialist", "Rijal expert"
) INHERITS (app.users);
...
-- Enforce role consistency in child tables (disjoint specialization constraint)
...
-- Study sets: any user can own → reference app.users (polymorphic ownership)
...
-- Assignments: no direct user FK (references circles and sets)
...
-- Assignment targets: only students are assigned → reference app.students
...
-- Progress: only students have progress → reference app.students
...
-- Review sessions: student is reviewed, teacher reviews
-- student_id: only students are reviewed → app.students
-- reviewer_id: only teachers review (or NULL for self-study) → app.teachers
...
-- Notes: any user can write notes → reference app.users (polymorphic)
...
-- Audit log: any user can trigger audits → reference app.users (polymorphic)
CREATE TABLE app.audit_log (                      -- shadow table (req 4b)
    audit_id   bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    table_name text NOT NULL,
    row_key    text NOT NULL,
    changed_by integer REFERENCES app.users(user_id),  -- set via set_config in API
    old_value  jsonb,
    new_value  jsonb,
...
-- FROM app.enrollments e
-- JOIN app.students u    ON u.user_id = e.student_id  -- Changed to app.students
